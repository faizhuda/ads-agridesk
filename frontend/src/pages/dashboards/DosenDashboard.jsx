...
import { listPendingDosen, listHistoryDosen, signSurat, rejectDosenSurat } from '../../api'
...
export default function DosenDashboard() {
  ...
  async function loadSurat() {
    ...
    try {
      const data = tab === 'pending'
        ? await listPendingDosen(token)
        : await listHistoryDosen(token)
      setSuratList(data)
    } catch (err) {
      console.error(err)
    } finally {
  ...
  async function handleSign(suratId) {
    setActionLoading(suratId)
    setMessage('')
    try {
      await signSurat(token, suratId, 'signatures/dosen_default.png')
      setMessage(`Surat #${suratId} berhasil ditandatangani`)
      loadSurat()
    } catch (err) {
  ...
  async function handleReject() {
    ...
    setMessage('')
    try {
      await rejectDosenSurat(token, suratId, reason)
  ...
  return (
    <div>
      <div className="section-header">
        <h2>{tab === 'pending' ? 'Menunggu Tanda Tangan' : 'Riwayat Tanda Tangan'}</h2>
        <div style={{ display: 'flex', gap: '8px' }}>
          <div className="tab-group">
            <button className={`tab ${tab === 'pending' ? 'tab-active' : ''}`} onClick={() => setTab('pending')}>Pending</button>
            <button className={`tab ${tab === 'history' ? 'tab-active' : ''}`} onClick={() => setTab('history')}>Riwayat</button>
          </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          <p>{tab === 'pending' ? 'Tidak ada surat yang menunggu tanda tangan.' : 'Belum ada riwayat.'}</p>
        </div>
      ) : (
                  ...
                  {tab === 'pending' && (
                    <td className="action-cell">
                      <button
                        ...
                        onClick={() => handleSign(s.surat_id)}
                        disabled={actionLoading === s.surat_id}
                      >
                        {actionLoading === s.surat_id ? '...' : 'Tanda Tangan'}
                      </button>
                      <button
        ...
        <div className="modal-overlay" onClick={() => setRejectModal({ open: false, suratId: null, reason: '' })}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              <button className="btn btn-secondary" onClick={() => setRejectModal({ open: false, suratId: null, reason: '' })}>Batal</button>
              <button className="btn btn-danger" onClick={handleReject} disabled={!rejectModal.reason.trim()}>Tolak Surat</button>
            </div>
          </div>
        </div>
      )}
